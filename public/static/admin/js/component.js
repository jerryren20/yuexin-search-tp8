...
Vue.component("Single", {
    ...
    methods: {
        ...
        // 处理直链输入
        handleDirectUrlInput() {
            if (this.directUrl && this.directUrl.trim()) {
                // 简单的URL格式验证
                const urlPattern = /^(https?:\/\/).*\.(jpg|jpeg|png|gif|bmp|webp)(\?.*)?$/i;
                if (urlPattern.test(this.directUrl.trim())) {
                    this.$emit('input', this.directUrl.trim());
                    this.directUrl = ''; // 清空输入框
                } else {
                    this.$message.error('请输入有效的图片链接地址');
                }
            }
        },
    }
