...
class Source extends QfShop
{
    ...
    public function getList(array $data)
    {
        ...
        if (!empty($data['title'])) {
            ...
            // 尝试使用全文索引（仅当 search_type=0 或未指定时）
            // 且仅当 title 长度大于 1 时
            $useFulltext = false;
            if ((!isset($data['search_type']) || $data['search_type'] == 0) && mb_strlen($title) > 1) {
                // 检查是否可以使用全文索引
                // 注意：这里需要确保数据库已添加 FULLTEXT 索引
                // 由于 MySQL 的 MATCH AGAINST 在布尔模式下支持操作符，我们可以简单处理
                $useFulltext = true;
            }
            ...
            if($search_type == 0){
                if ($useFulltext) {
                    // 使用全文索引
                    // 转换关键词为布尔模式格式，例如 "+keyword*"
                    // 这里简单处理，直接使用自然语言模式
                    $query = $this->whereRaw("MATCH(title, description) AGAINST(:title IN NATURAL LANGUAGE MODE)", ['title' => $title])
                                  ->where($map);
                    // 全文索引会自动按相关性排序
                    $order = []; 
                } else {
                    $map[] = ['title|description', 'like', '%' . $title . '%'];
