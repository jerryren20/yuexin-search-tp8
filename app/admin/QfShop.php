...
abstract class QfShop
{
    ...
    protected function exportExcelData($data)
    {
        ...
        $PHPSheet->getStyle('A1')->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);    //水平方向上对齐
        $PHPSheet->getStyle('A1')->getAlignment()->setVertical(Alignment::VERTICAL_CENTER);       //垂直方向上中间居中
        $PHPSheet->getStyle('A1')->getAlignment()->setVertical(Alignment::VERTICAL_CENTER);       //垂直方向上中间居中
        ...
        for ($column = 0; $column < count($excelField); $column++) {
            ...
            for ($line = 0; $line < count($datalist); $line++) {
                ...
                $PHPSheet->setCellValueExplicit($this->excelCells[$column] . ($line + 3), $string, DataType::TYPE_STRING);
            }
        }
        //***********************画出单元格边框*****************************
        $styleArray = array(
        ...
        $PHPSheet->getStyle('A1:' . $this->excelCells[count($excelField) - 1] . (count(
            $datalist
        ) + 2))->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER)->setVertical(Alignment::VERTICAL_CENTER)->setVertical(Alignment::VERTICAL_CENTER);
        //设置全部字体
        $PHPSheet->getStyle('A1:' . $this->excelCells[count($excelField) - 1] . (count(
            $datalist
        ) + 2))->getFont()->setName('微软雅黑');
        $PHPWriter = new Xlsx($spreadsheet);
